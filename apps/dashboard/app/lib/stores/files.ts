...
export class FilesStore {
  ...
  async #init() {
    ...
    // Set up file watcher - only if webcontainer.internal exists
    try {
      if (webcontainer.internal && typeof webcontainer.internal.watchPaths === 'function') {
        ...
        console.log('[FilesStore] ⚠️ WebContainer.internal.watchPaths not available, skipping file watching');
      }
    } catch (error) {
      console.warn('[FilesStore] ⚠️ Error setting up file watcher (non-critical):', error);
    }
